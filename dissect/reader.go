...
type Reader struct {
	...
	PositionUpdates []PositionUpdate `json:"positionUpdates,omitempty"`
	AmmoUpdates     []AmmoUpdate     `json:"ammoUpdates,omitempty"`
	...
	ScoreUpdates    []ScoreUpdate    `json:"scoreUpdates,omitempty"`
	DeathTimings    []DeathTiming    `json:"deathTimings,omitempty"`
	CameraFrames    []CameraFrame    `json:"cameraFrames,omitempty"`
...
func NewReader(in io.Reader) (r *Reader, err error) {
	...
	r.Listen([]byte{0x77, 0xCA, 0x96, 0xDE}, readAmmo)
	return r, err
}
...
// reconcileOperatorSwaps uses the header's RoleName (which reflects the final
// operator after any attacker swaps) to fix the Operator field that readPlayer
// sets to the initial (pre-swap) pick. This is needed because readAtkOpSwap
// cannot reliably match players in Y10S4+ replays.
func (r *Reader) reconcileOperatorSwaps() {
	// Build reverse map: uppercase ASCII-folded operator name -> Operator ID
	nameToOp := make(map[string]Operator, len(_Operator_map))
	for op, name := range _Operator_map {
		nameToOp[strings.ToUpper(foldAccents(name))] = op
	}
	for i, p := range r.Header.Players {
		if p.RoleName == "" {
			continue
		}
		headerName := strings.ToUpper(foldAccents(p.RoleName))
		currentName := strings.ToUpper(foldAccents(p.Operator.String()))
		if currentName == headerName {
			continue
		}
		newOp, ok := nameToOp[headerName]
		if !ok {
			log.Warn().Str("player", p.Username).Str("roleName", p.RoleName).Msg("unknown operator in header")
			continue
		}
		log.Debug().
			Str("player", p.Username).
			Str("from", p.Operator.String()).
			Str("to", p.RoleName).
			Msg("reconcile_operator_swap")
		r.Header.Players[i].InitialOperator = p.Operator
		r.Header.Players[i].Operator = newOp
...
// foldAccents replaces common accented Latin characters with their ASCII base.
func foldAccents(s string) string {
	var b strings.Builder
	for _, r := range s {
		switch {
		case r >= 0x00C0 && r <= 0x00C5: // À-Å → A
			b.WriteByte('A')
		case r >= 0x00E0 && r <= 0x00E5: // à-å → a
			b.WriteByte('a')
		case r >= 0x00C8 && r <= 0x00CB: // È-Ë → E
			b.WriteByte('E')
		case r >= 0x00E8 && r <= 0x00EB: // è-ë → e
			b.WriteByte('e')
		case r >= 0x00CC && r <= 0x00CF: // Ì-Ï → I
			b.WriteByte('I')
		case r >= 0x00EC && r <= 0x00EF: // ì-ï → i
			b.WriteByte('i')
		case r >= 0x00D2 && r <= 0x00D6: // Ò-Ö → O
			b.WriteByte('O')
		case r >= 0x00F2 && r <= 0x00F6: // ò-ö → o
			b.WriteByte('o')
		case r >= 0x00D9 && r <= 0x00DC: // Ù-Ü → U
			b.WriteByte('U')
		case r >= 0x00F9 && r <= 0x00FC: // ù-ü → u
			b.WriteByte('u')
		case r == 0x00D1: // Ñ → N
			b.WriteByte('N')
		case r == 0x00F1: // ñ → n
